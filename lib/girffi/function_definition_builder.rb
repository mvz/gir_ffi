module GirFFI
  ...
  class FunctionDefinitionBuilder
    ...
    def process_inout_arg a
      ...
      case a.type.tag
      when :int
	@pre << "#{prevar} = GirFFI::ArgHelper.int_to_inoutptr #{a.name}"
	...
	when :utf8
	  ...
	  @pre << "#{prevar} = GirFFI::ArgHelper.string_array_to_inoutptr #{a.name}"
	  @post << "#{postvar} = GirFFI::ArgHelper.outptr_to_string_array #{prevar}, #{a.name}.nil? ? 0 : #{a.name}.size"
	else
	  raise NotImplementedError
	end
      else
	...
	@inargs << a.name
	prevar = new_var
	@pre << "#{prevar} = GirFFI::ArgHelper.object_to_inptr #{a.name}"
	@callargs << prevar
	return
      end
      @inargs << a.name
